...
| Claude in Chrome / Control Chrome connector | TESTED S65 — WORKS BUT FLAKY | Confirmed genuine live cell-editing in Google Sheets works when paired (proved on one cell edit, verified via re-read). Pairing itself dropped unprompted twice mid-session, needed `tabs_context_mcp{createIfEmpty:true}` to re-establish. Usable for occasional live edits; not reliable enough yet to depend on for anything time-sensitive. Recommendation: Artie Apps Script Web App write endpoint (same pattern as `aura_thai_finance`) is the durable fix if recurring sheet-writing becomes a regular need. |
